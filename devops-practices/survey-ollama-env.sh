...
if command -v ollama &>/dev/null; then
  ...
  if curl -s http://localhost:11434/api/tags &>/dev/null; then
    ...
    INSTALLED_MODELS=$(curl -s http://localhost:11434/api/tags | { grep -o '"name":"[^"]*"' || true; } | cut -d'"' -f4 | paste -sd ', ' -)
    if [[ -n "$INSTALLED_MODELS" ]]; then
      echo "  Installed models: $INSTALLED_MODELS"
    else
      echo "  Installed models: (none yet — recommendations follow in step 5)"
    fi
  else
...
if [[ "$OLLAMA_RUNNING" == true ]]; then
  CURRENT_INSTALLED=$(curl -s http://localhost:11434/api/tags 2>/dev/null | { grep -o '"name":"[^"]*"' || true; } | cut -d'"' -f4 | tr '\n' ' ')
