...
- [x] **RB-6 (optional): Silence redundant MPS float64 warning** (v4.1.0) — the MPS float64 CPU fallback in `transcription_engine/whisper_engine.py` now emits its user-facing WARNING once per process and logs subsequent occurrences at INFO level; behaviour unchanged. Folds in the long-standing "Suppress or optimise MPS float64 warnings" item above. (Effort: XS, Haiku)
