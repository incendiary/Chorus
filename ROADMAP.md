...
- [x] **RA-5: Test hardware_survey.py's detection and recommendation logic** (v4.0.1) — coverage raised from 14% to 100%; added 56 tests covering RAM/CPU/GPU detection (macOS `sysctl`, Linux `/proc/meminfo`, torch CUDA/MPS, `nvidia-smi` fallback), graceful fallback on missing/erroring/malformed subprocess output, and the whisper-model/device/parallelism recommendation branches across NVIDIA, Apple Silicon, and CPU-only hardware profiles. (Effort: M) — files: `tests/test_hardware_survey.py`.
...
- [x] **RB-4: Version the bundle schema and add a contract test** — `bundle.json`'s `meta` now carries `chorus_version` (read from the root `VERSION` file) and `schema_version` (integer, currently 1, bumped only on renames/removals); `docs/CHORUS_FOR_LLMS.md` §5 updated to match, and a two-way contract test asserts every real bundle key appears in the documented example and vice versa, so contract drift now fails CI. (Effort: S, Haiku)
